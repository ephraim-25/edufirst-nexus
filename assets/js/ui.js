// EduFirst — Shared UI utilities (Vanilla JS)
...
const ICONS = {
  ...
  school: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m4 6 8-4 8 4"/><path d="m18 10 4 2v8a2 2 0 0 1-2 2h-2v-8h-4"/><path d="m2 12 4-2"/><path d="m6 8 0 14"/><path d="M14 22v-8h4"/><path d="M18 22V11"/></svg>',
  bag: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 10a4 4 0 0 1-8 0"/><rect x="3" y="6" width="18" height="15" rx="2"/><path d="M3 10h18"/></svg>',
  ...
  chart: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 16V8"/><path d="M11 16V5"/><path d="M15 16v-6"/><path d="M19 16v-3"/></svg>',
  ...
  globe: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20M12 2a14.5 14.5 0 0 1 0 20M2 12h20"/></svg>',
  ...
  edit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4z"/></svg>',
  ...
  eye: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7"/><circle cx="12" cy="12" r="3"/></svg>',
  ...
  alert: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0M12 9v4M12 17h.01"/></svg>',
...
window.icon = (name, cls = "w-5 h-5") => {
  const svg = ICONS[name] || "";
  return svg.replace("<svg ", `<svg class="${cls}" `);
};

// ============ MULTI-EXTENSION SWITCHER ============
const CAMPUSES = [
...
window.getCurrentCampus = () => {
  const id = localStorage.getItem("edufirst_campus") || "nord";
  return CAMPUSES.find((c) => c.id === id) || CAMPUSES[0];
};

window.renderCampusSwitcher = (containerId) => {
  const container = document.getElementById(containerId);
  if (!container) return;
  const current = getCurrentCampus();
  container.innerHTML = `
    <div class="relative">
      <button id="campus-btn" class="flex items-center gap-2 px-3 py-2 rounded-lg bg-white shadow-surface text-sm font-medium hover:bg-primary-soft transition">
        <span class="w-2 h-2 rounded-full bg-primary"></span>
        <span class="text-foreground">${current.name}</span>
        <span class="text-muted text-xs hidden sm:inline">· ${current.students} élèves</span>
        ${icon("chevronDown", "w-4 h-4 text-muted")}
      </button>
      <div id="campus-menu" class="hidden absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-lg border border-border z-50 overflow-hidden">
        ${CAMPUSES.map((c) => `
          <button data-campus="${c.id}" class="campus-option w-full text-left px-4 py-3 hover:bg-primary-soft transition flex items-center justify-between ${c.id === current.id ? "bg-primary-soft" : ""}">
            <div>
              <div class="text-sm font-medium text-foreground">${c.name}</div>
              <div class="text-xs text-muted">${c.students} élèves · ${c.city}</div>
            </div>
            ${c.id === current.id ? icon("check", "w-4 h-4 text-primary") : ""}
          </button>
        `).join("")}
      </div>
    </div>
  `;
  const btn = document.getElementById("campus-btn");
  const menu = document.getElementById("campus-menu");
  btn.addEventListener("click", (e) => { e.stopPropagation(); menu.classList.toggle("hidden"); });
  document.addEventListener("click", () => menu.classList.add("hidden"));
  container.querySelectorAll(".campus-option").forEach((b) => {
    b.addEventListener("click", () => {
      localStorage.setItem("edufirst_campus", b.dataset.campus);
      location.reload();
    });
  });
};
...
// ============ TABS ============
window.initTabs = (containerSelector) => {
...
window.toast = (message, type = "success") => {
  const colors = { success: "bg-emerald-500", error: "bg-red-500", info: "bg-primary" };
  const el = document.createElement("div");
  el.className = `fixed bottom-6 right-6 ${colors[type]} text-white px-5 py-3 rounded-xl shadow-2xl z-[200] slide-up font-medium text-sm`;
...
// ============ FORMAT ============
window.formatFCFA = (n) => `${(n > 0 ? "+" : "")}${n.toLocaleString("fr-FR")} FCFA`;
window.formatDate = (d) => new Date(d).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" });
